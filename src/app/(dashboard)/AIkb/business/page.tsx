...
export default function BusinessKnowledgePage() {
  ...
  const handleAddSubmit = async (type: 'FILE' | 'FOLDER') => {
    ...
    try {
      ...
      message.success(type === 'FILE' ? 'File added' : 'Folder created');
  ...
  const handleRealFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!kbRes.ok) throw new Error('Failed to save file record');
      
      message.success({ content: 'File uploaded', key: 'uploading' });
      fetchItems(currentFolderId);
    } catch {
  ...
  const handleOpenMove = async (item: any, e: React.MouseEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      setAllFolders([{id: 'root', title: 'Root'}, ...data.filter((d: any) => d.type === 'FOLDER' && d.id !== item.id)]);
    } catch {}
  };
  ...
  return (
    <div className="w-full h-full bg-[#f8f9fc] flex flex-col overflow-hidden">
      <div className="px-8 pt-7 pb-2 shrink-0">
        <h1 className="text-xl font-black text-gray-900 tracking-tight">Business Knowledge</h1>
        <p className="text-xs text-gray-400 mt-1">Manage company, client, and industry knowledge documents</p>
      </div>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
          ...
          <div className="flex gap-2">
            <button onClick={() => setIsAddFolderVisible(true)} className="px-4 py-2 bg-white text-gray-600 border border-gray-200 rounded-lg text-xs font-bold flex items-center gap-2 hover:bg-gray-50 shadow-sm transition-colors">
              <FolderPlus className="w-3.5 h-3.5 text-blue-500" /> New Folder
            </button>
            <button onClick={() => setIsAddFileVisible(true)} className="px-4 py-2 bg-white text-gray-600 border border-gray-200 rounded-lg text-xs font-bold flex items-center gap-2 hover:bg-gray-50 shadow-sm transition-colors">
              <PenTool className="w-3.5 h-3.5 text-emerald-500" /> Manual Entry
            </button>
        ...
        <div className="bg-white border border-gray-100 rounded-xl overflow-hidden flex-1 flex flex-col min-h-0">
          {loading ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center min-h-[300px] text-gray-400">
              ...
              <p className="text-sm">Create a folder or upload files using the buttons above</p>
            </div>
          ) : (
              ...
              {items.map(item => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    {item.fileUrl && (
                      <button onClick={(e) => { e.stopPropagation(); window.open(item.fileUrl, '_blank'); }} className="p-2 text-gray-400 hover:text-emerald-500 hover:bg-emerald-50 rounded-lg transition-all shrink-0" title="View / Download">
                        <Download className="w-4 h-4" />
                      </button>
                    )}
                    <button onClick={(e) => handleOpenMove(item, e)} className="p-2 text-gray-400 hover:text-blue-500 hover:bg-blue-50 rounded-lg transition-all shrink-0" title="Move">
                      <ArrowRightLeft className="w-4 h-4" />
                    </button>
                    <button onClick={(e) => handleDeleteItem(item.id, e)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all shrink-0" title="Delete">
      ...
      {/* Add Text Modal */}
      <Modal title="Manual Text Entry" open={isAddFileVisible} onCancel={() => setIsAddFileVisible(false)} footer={[
        <Button key="cancel" onClick={() => setIsAddFileVisible(false)}>Cancel</Button>,
        <Button key="submit" type="primary" loading={isSubmitting} onClick={() => handleAddSubmit('FILE')} className="bg-emerald-600">Save</Button>,
      ]} width={600}>
        <div className="space-y-4 mt-4">
          <div>
            <label className="block text-xs font-bold text-gray-700 mb-1">Title</label>
            <Input placeholder="e.g. Company Vision & Values" value={addForm.title} onChange={e => setAddForm(prev => ({ ...prev, title: e.target.value }))} />
          </div>
          <div>
            <label className="block text-xs font-bold text-gray-700 mb-1">Content (Markdown supported)</label>
            <Input.TextArea placeholder="Paste or type content..." value={addForm.content} onChange={e => setAddForm(prev => ({ ...prev, content: e.target.value }))} rows={8} />
          </div>
        </div>
      </Modal>

      {/* Add Folder Modal */}
      <Modal title="New Folder" open={isAddFolderVisible} onCancel={() => setIsAddFolderVisible(false)} footer={[
        <Button key="cancel" onClick={() => setIsAddFolderVisible(false)}>Cancel</Button>,
        <Button key="submit" type="primary" loading={isSubmitting} onClick={() => handleAddSubmit('FOLDER')} className="bg-emerald-600">Create</Button>,
      ]} width={400}>
        <div className="mt-4">
          <label className="block text-xs font-bold text-gray-700 mb-1">Folder Name</label>
          <Input placeholder="Enter name..." value={addForm.title} onChange={e => setAddForm(prev => ({ ...prev, title: e.target.value }))} onPressEnter={() => handleAddSubmit('FOLDER')} />
        </div>
      </Modal>

      {/* Move Modal */}
      <Modal title="Move to..." open={isMoveVisible} onCancel={() => setIsMoveVisible(false)} footer={null} width={400}>
