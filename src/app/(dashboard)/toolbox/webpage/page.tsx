      ...
      <div className="max-w-3xl mx-auto p-8 pb-20">
        ...
        <div className="space-y-5">
          <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
            <div className="px-5 py-3 bg-gray-50/50 border-b border-gray-100"><h3 className="text-xs font-bold text-gray-600">Topic *</h3></div>
            <div className="p-5">
          ...
          <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
            <div className="px-5 py-3 bg-gray-50/50 border-b border-gray-100"><h3 className="text-xs font-bold text-gray-600">Design Style</h3></div>
            <div className="p-5">
          ...
          <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
            <div className="px-5 py-3 bg-gray-50/50 border-b border-gray-100"><h3 className="text-xs font-bold text-gray-600">Number of Pages</h3></div>
            <div className="p-5">
          ...
          <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
            <div className="px-5 py-3 bg-gray-50/50 border-b border-gray-100 flex items-center justify-between">
              <h3 className="text-xs font-bold text-gray-600">Background (Optional)</h3>
              <button onClick={() => setKbSelectorOpen(true)} className="text-[11px] font-medium text-teal-600 bg-teal-50 px-2 py-1 rounded hover:bg-teal-100 flex items-center gap-1"><Database className="w-3 h-3" /> Select from KB</button>
            </div>
            <div className="p-5">
          ...
          <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
            <div className="px-5 py-3 bg-gray-50/50 border-b border-gray-100"><h3 className="text-xs font-bold text-gray-600">Extra Requirements (Optional)</h3></div>
            <div className="p-5">
...
You can enter edit instructions on the right, or switch to manual mode to edit content directly.

💡 Click image placeholder areas to replace with real images.` }]);
...
<\/script></body></html>`;
  ...
  const sendWebChat = (msg: string) => {
    ...
    fetch('/api/toolbox/webpage', {
      ...
      if (data.success && data.site) { setWebResult(data.site); setWebChatHistory(prev => [...prev, { role: 'bot', content: '✅ Updated! Check the preview.' }]); }
      ...
      <div className="h-14 bg-white border-b flex items-center justify-between px-6 shrink-0 shadow-sm z-20">
        <div className="flex bg-gray-100 p-1 rounded-xl">
          <button onClick={() => setWebEditorMode('ai')} className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-2 ${webEditorMode === 'ai' ? 'bg-white shadow-sm text-teal-600' : 'text-gray-500'}`}>
            <MessageSquare className="h-3.5 w-3.5" /> AI Edit
          </button>
          <button onClick={() => setWebEditorMode('manual')} className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-2 ${webEditorMode === 'manual' ? 'bg-white shadow-sm text-teal-600' : 'text-gray-500'}`}>
            <Edit3 className="h-3.5 w-3.5" /> Manual Edit
          </button>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Preview: <span className="text-gray-700">{activePage?.title}</span></span>
          {webPublishedUrl && (
          ...
          <button onClick={async () => {
            ...
            {webPublishing ? <><Loader2 className="w-3.5 h-3.5 animate-spin" /> Publishing</> : <><Globe className="w-3.5 h-3.5" /> Publish Site</>}
          </button>
          <button onClick={() => setWebResult(null)} className="px-3 py-1.5 bg-gray-100 text-gray-500 rounded-lg text-xs font-bold hover:bg-gray-200">Regenerate</button>
        </div>
      </div>
        ...
        {webEditorMode === 'manual' && (
          <aside className="w-56 bg-white border-r flex flex-col shrink-0">
            <div className="p-4 border-b flex items-center justify-between bg-gray-50/50">
              <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider flex items-center gap-2"><Layout className="h-3 w-3" /> Pages</h3>
              <button onClick={() => {
            ...
            <div className="p-3 bg-gray-50 text-[9px] text-gray-400 text-center border-t">Click image placeholders to replace images</div>
          </aside>
        )}
        ...
        {webEditorMode === 'ai' && (
          <aside className="w-80 bg-white border-l flex flex-col shrink-0 shadow-lg">
            <div className="p-4 border-b flex items-center justify-between bg-teal-50/30">
              <div className="flex items-center gap-2"><MessageSquare className="h-4 w-4 text-teal-500" /><h3 className="font-bold text-sm text-gray-800">AI Design Assistant</h3></div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {webChatHistory.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center px-4">
                  ...
                  <p className="text-xs text-gray-400 leading-relaxed">Try: &quot;Change the hero background to dark blue&quot;, &quot;Add a contact form&quot; or &quot;Convert the course list to card layout&quot;</p>
                </div>
              ) : (
